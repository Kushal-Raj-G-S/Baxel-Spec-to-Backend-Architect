...
def generate_code_skeleton(spec) -> Dict[str, str]:
    ...
    models_code += "from sqlalchemy import Column, String, Integer, DateTime, ForeignKey, Boolean, Enum as SAEnum\n"
    ...
    for table in spec.database.tables:
        ...
        for col in table.columns:
            ...
            if col.enum_values:
                type_str = "SAEnum(" + ", ".join(repr(value) for value in col.enum_values) + ")"
    ...
    for ep in spec.endpoints:
        base_name = (ep.summary or ep.path or "endpoint").lower()
        normalized_name = re.sub(r"[^0-9a-zA-Z_]+", "_", base_name).strip("_")
        if not normalized_name or normalized_name[0].isdigit():
            normalized_name = f"endpoint_{normalized_name}" if normalized_name else "endpoint"
        routers_code += f"@router.{ep.method.lower()}({ep.path!r}, summary={ep.summary!r})\n"
        ...
        routers_code += f"    return {{'message': {ep.summary!r}}}\n\n"
...
def generate_migration_sql(spec) -> str:
    ...
    for table in spec.database.tables:
        ...
        for col in table.columns:
            ...
            if col.enum_values:
                enum_values_sql = ", ".join(f"'{value}'" for value in col.enum_values)
                constraints.append(f"CHECK ({col.name} IN ({enum_values_sql}))")
...
async def create_project(payload: ProjectCreate, user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    existing_project = (
        ...
        .filter(ProjectModel.user_id == uid)
        .filter(func.lower(func.trim(ProjectModel.name)) == normalized_name)
        .first()
    )
    if existing_project:
        return {
